...
/** Safe i18n.t() — returns fallback if i18n is not initialized */
function t(key: string, fallback: string): string {
  try { return i18n.isInitialized ? i18n.t(key, fallback) : fallback }
        ...
        <div className="flex min-h-screen items-center justify-center bg-bg-primary p-6">
          <div className="max-w-md text-center">
            ...
            <p className="mb-6 text-sm text-text-muted">
              {this.state.error?.message || t('error.unexpected', 'An unexpected error occurred')}
            </p>
            <button
